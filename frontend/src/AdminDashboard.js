...
export default function AdminDashboard({ token, userName, onLogout }) {
  ...
  const handleCreate = async () => {
    if (!form.name || !form.email || !form.password) {
      setMsg({ text: "Please fill in all three fields.", ok: false });
      return;
    }
    ...
    try {
      ...
      setMsg({
        text: `✅ User "${form.name}" created successfully!`,
        ok: true,
      });
      ...
      setMsg({
        text: e.response?.data?.message || "❌ Failed to create user.",
        ok: false,
      });
    } finally {
      setCreating(false);
  ...
  const handleDeleteUser = async (user) => {
    if (
      !window.confirm(
        `Are you sure you want to delete "${user.name}"? All their data will be removed.`,
      )
    )
      return;
  ...
  const viewReport = async (user) => {
    ...
    try {
      ...
      setReport({
        error: e.response?.data?.message || "Could not load report.",
      });
    } finally {
      setRepLoading(false);
    }
    ...
    <div className="shell">
      ...
      <aside className="sidebar">
        <div className="sb-brand">
          ...
          <span className="sb-name">VP FinTrack</span>
        </div>
        <div className="sb-user">
          <div className="sb-avatar sb-admin-av">
        ...
        <nav className="sb-nav">
          {navItems.map((n) => (
            <button
              key={n.id}
              className={`sb-link ${tab === n.id ? "sb-active" : ""}`}
              onClick={() => setTab(n.id)}
            >
      ...
      <main className="main">
                  ...
                  {users.map((u) => (
                    <tr key={u.id}>
                      ...
                      <td>
                        ...
                        >
                          <div className="tav">{u.name[0].toUpperCase()}</div>
                          {u.name}
                        </div>
                      </td>
                      ...
                      <td>
                        <span
                          className={`badge ${u.role === "ADMIN" ? "b-y" : "b-b"}`}
                        >
                          {u.role}
                        </span>
                      </td>
        ...
        {tab === "create" && (
          <div className="page fade">
            <div className="ph">
              <div>
                <h2 className="pt">Create User</h2>
                <p className="ps">Add a new member to the system</p>
              </div>
            </div>
        ...
        {tab === "report" && (
          <div className="page fade">
            <div className="ph">
              <div>
                <h2 className="pt">
                  {selUser ? `${selUser.name}'s Report` : "User Report"}
                </h2>
                <p className="ps">
                  {selUser?.email || "Select a user from the Users tab"}
                </p>
              </div>
              {selUser && (
                <button
              ...
              <>
                <div className="stats">
                  ...
                  <div className="sc sc-g">
                    <div className="sl">Total Income</div>
                    <div className="sv">
                      ₹{(report.totalIncome || 0).toLocaleString()}
                    </div>
                  </div>
                  <div className="sc sc-y">
                    <div className="sl">Transactions</div>
                    <div className="sv">{report.transactionCount}</div>
                  </div>
                </div>
                    ...
                    <tbody>
                      ...
                      {report.transactions.map((t) => (
                        <tr key={t.id}>
                          ...
                          <td
                            className={t.type === "income" ? "amt-g" : "amt-r"}
                          >
                            ₹{t.amount.toLocaleString()}
                          </td>
                          <td>
                            <span
                              className={`badge ${t.type === "income" ? "b-g" : "b-r"}`}
                            >
                              {t.type}
                            </span>
                          </td>
            ...
            {selUser && report?.error && (
              <div className="empty-state">
                <p style={{ color: "#ff4d6a" }}>{report.error}</p>
              </div>
            )}
          </div>
        )}
